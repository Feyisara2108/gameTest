...
// CONTRACT 1: SchoolManager ERC20 Token
contract SchoolManager {
    // BASIC TOKEN INFO
    string public constant name = "SchoolManager";
    string public constant symbol = "SCH";
    uint8 public constant decimals = 18;
    uint256 public totalSupply;

    mapping(address => uint256) public balanceOf; //this is simply asking for who owns how many tokens?

    mapping(address => mapping(address => uint256)) public allowance; //Who can spend the tokens
    ...
    // TRANSFER FROM: Spend tokens on behalf of someone/me
    ...
    // BUY TOKENS: Send ETH, get SchoolManagers (1:1 ratio)
    function buyTokens() public payable {
        if (msg.value == 0) revert NoETHToSend();

        //tokens givens equal to ETH sent (1 ETH = 1 Token)
    ...
    //SELL TOKENS: Send tokens back, get ETH (for staffs to be able to cash out real funds)
    ...
    // MINT: Create new tokens (simply for owner only)
...
contract SchoolToken {
    ...
    constructor(address _tokenAddress) {
        token = SchoolManager(_tokenAddress);
    ...
    function payStaff(address _staff) external onlyPrincipal {
        ...
        if (!token.transfer(_staff, s.salary)) revert PaymentFailed();
    }
